...
*   **Action Required Tab (`LeadAgingPanel.tsx`):** This is the core "Actionable Insight" differentiator. To ensure it accurately shows *present-day* tasks, it consciously ignores the "Month" filter by pulling the `branchFilteredLeads` list. It calculates a static "Today" based on the absolute latest logging in the ledger, and flags any un-converted lead that has sat dormant for more than 7 days, tallying the exact pipeline revenue sitting at risk.
